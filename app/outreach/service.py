...
from app.database.models import (
    AIAnalysis, AIStatus, DentalFinding, Patient, WhatsAppOutreach, WhatsAppOutreachStatus,
)
...
async def latest_eligible_finding(session: AsyncSession, patient_id):
    analysis = await session.scalar(
        select(AIAnalysis).where(
            AIAnalysis.patient_id == patient_id, AIAnalysis.status == AIStatus.COMPLETED
...
async def build_outreach(session: AsyncSession, *, patient: Patient, analysis: AIAnalysis,
                         finding: DentalFinding, immediate: bool = False,
                         include_image: bool = False) -> WhatsAppOutreach:
    analysis_at = analysis.completed_at or analysis.requested_at or datetime.now(UTC)
    timing = FollowupTimingEngine().calculate(
        finding_type=finding.finding_type, tooth_fdi=finding.tooth_code,
        analysis_at=analysis_at,
        review_required=bool((finding.provenance or {}).get("review_required")),
    )
    row = WhatsAppOutreach(
        patient_id=patient.id, analysis_id=analysis.id, finding_id=finding.id,
        tooth_fdi=finding.tooth_code, finding_type=finding.finding_type,
        ...
        message=await armenian_message(timing), language="hy-AM",
        status=WhatsAppOutreachStatus.QUEUED if immediate else WhatsAppOutreachStatus.SCHEDULED,
        timing_reason=timing.timing_reason, timing_policy_rule_id=timing.policy_rule_id,
        timing_policy_version=timing.policy_version, clinic_timezone=timing.clinic_timezone,
...
async def schedule_analysis_outreach(session: AsyncSession, analysis: AIAnalysis,
                                     findings: list[DentalFinding]) -> int:
    ...
    for finding in findings:
        ...
        if existing:
            continue
        await build_outreach(
            session, patient=patient, analysis=analysis, finding=finding
        )
        count += 1
    return count
