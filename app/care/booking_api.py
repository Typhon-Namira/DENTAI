...
from app.care.booking_links import booking_token, booking_url, decode_booking_token
...
def _uuid(payload: dict, key: str) -> uuid.UUID | None:
...
def _utc(value: datetime) -> datetime:
    if value.tzinfo is None:
        raise AppError("BOOKING_TIMEZONE_REQUIRED", "The selected time must include a timezone.", 422)
...
def _requested_message(language: str, slot: datetime, timezone_name: str) -> str:
    ...
    if language == "hy":
        return f"Ձեր {label} ({timezone_name}) ստուգման ժամը գրանցվել է և սպասում է բժշկի հաստատմանը։ Բժշկի պատասխանից հետո անմիջապես կգրենք ձեզ։"
    if language == "ru":
        return f"Ваш запрос на осмотр {label} ({timezone_name}) зарегистрирован и ожидает подтверждения врача. Мы сразу напишем вам после ответа врача."
    return f"Your requested check-up time, {label} ({timezone_name}), has been registered and is waiting for the doctor's confirmation. We'll message you as soon as the doctor responds."


def _approved_message(language: str, slot: datetime, timezone_name: str) -> str:
    label = _slot_label(slot, timezone_name)
    if language == "hy":
        return f"Բժիշկը հաստատել է ձեր ստուգման ժամը՝ {label} ({timezone_name})։ Սպասում ենք ձեզ կլինիկայում։"
    if language == "ru":
        return f"Врач подтвердил ваш прием: {label} ({timezone_name}). Ждем вас в клинике."
    return f"Your doctor has confirmed your check-up for {label} ({timezone_name}). We look forward to seeing you at the clinic."


def _range_message(language: str, start: datetime, end: datetime, timezone_name: str, url: str, note: str | None) -> str:
    start_label = _slot_label(start, timezone_name)
    end_label = _slot_label(end, timezone_name)
    note_line = f"\n{note.strip()}" if note and note.strip() else ""
    if language == "hy":
        return f"Բժիշկն առաջարկում է նոր ժամանակային միջակայք՝ {start_label}–{end_label} ({timezone_name})։{note_line}\nԱյս միջակայքից ընտրեք ազատ ժամը՝ {url}"
    if language == "ru":
        return f"Врач предлагает новый интервал: {start_label}–{end_label} ({timezone_name}).{note_line}\nВыберите свободное время в этом интервале: {url}"
    return f"Your doctor suggests a new time window: {start_label}–{end_label} ({timezone_name}).{note_line}\nChoose an available time in this window: {url}"


async def _find_patient_by_phone(session: AsyncSession, phone: str, branch_id: uuid.UUID) -> Patient | None:
    normalized = normalize_phone(phone)
    rows = (
        await session.scalars(
            select(Patient).where(Patient.branch_id == branch_id, Patient.status != "ARCHIVED")
        )
    ).all()
    for patient in rows:
...
async def _conversation_for_patient(session: AsyncSession, patient_id: uuid.UUID) -> CareConversation | None:
    return await session.scalar(
        select(CareConversation)
        .where(CareConversation.patient_id == patient_id, CareConversation.status == "ACTIVE")
...
async def clinic_booking_link(
    ...
    return {
        "url": booking_url(clinic_id=ctx.clinic.id, branch_id=branch_id, doctor_id=doctor_id),
...
async def public_booking_form(
    ...
    async with resolver.session_factory(clinic)() as session:
        ...
        if not branch or not branch.is_active:
            raise AppError("BOOKING_BRANCH_UNAVAILABLE", "This clinic location is unavailable.", 404)
...
async def public_book(
    ...
    async with resolver.session_factory(clinic)() as session:
        ...
        fresh_by_utc = {slot.astimezone(UTC): slot for slot in fresh_slots}
        if selected not in fresh_by_utc:
            raise AppError("BOOKING_SLOT_UNAVAILABLE", "That time is no longer available. Please choose another slot.", 409)
        start_raw, end_raw = payload.get("window_start"), payload.get("window_end")
        if start_raw and end_raw:
            window_start = _utc(datetime.fromisoformat(str(start_raw)))
            window_end = _utc(datetime.fromisoformat(str(end_raw)))
            if not (window_start <= selected <= window_end):
                raise AppError("BOOKING_SLOT_OUTSIDE_DOCTOR_WINDOW", "Please select a time inside the doctor's suggested window.", 409)

        patient = await session.get(Patient, patient_id) if patient_id else None
        if not patient:
            if not body.first_name or not body.last_name or not body.phone:
                raise AppError("BOOKING_PATIENT_DETAILS_REQUIRED", "Name and phone are required for booking.", 422)
            try:
                normalized_phone = normalize_phone(body.phone)
            except ValueError as exc:
                raise AppError("BOOKING_PHONE_INVALID", "Use an international phone number.", 422) from exc
        ...
        try:
            ...
            raise AppError("BOOKING_SLOT_UNAVAILABLE", "That time was just booked. Please choose another slot.", 409) from exc
...
async def approve_booking_appointment(
    ...
    conversation = await ctx.session.get(CareConversation, row.conversation_id) if row.conversation_id else None
    phone = patient.whatsapp_phone or patient.phone
    ...
    if notification_status == "FAILED":
        raise AppError("APPOINTMENT_NOTIFICATION_FAILED", "The patient could not be notified, so the appointment remains pending.", 503)
    row.status = "APPROVED"
    ...
    if row.care_plan_item_id:
        ...
        if item:
            item.status = "BOOKED"
    await audit(ctx.session, ctx.user, "APPOINTMENT_APPROVED", "CareAppointment", row.id, row.branch_id)
    try:
        await ctx.session.commit()
    except IntegrityError as exc:
        await ctx.session.rollback()
        raise AppError("APPOINTMENT_COLLISION", "The selected time was booked concurrently.", 409) from exc
...
async def suggest_booking_range(
    ...
    if end <= start:
        raise AppError("INVALID_DOCTOR_WINDOW", "The end of the suggested window must be after its start.", 422)
    settings = await settings_for_branch(ctx.session, row.branch_id)
    patient = await ctx.session.get(Patient, row.patient_id)
    if not patient:
        raise AppError("PATIENT_NOT_FOUND", "Patient was not found.", 404)
    conversation = await ctx.session.get(CareConversation, row.conversation_id) if row.conversation_id else await _conversation_for_patient(ctx.session, patient.id)
    language = _language(conversation.language if conversation else language_for_phone(patient.whatsapp_phone or patient.phone, "en"))
    url = booking_url(
        clinic_id=ctx.clinic.id,
    ...
    message = _range_message(language, start, end, settings.timezone, url, body.note)
    ...
    if notification_status == "FAILED":
        raise AppError("APPOINTMENT_NOTIFICATION_FAILED", "The doctor's suggested range could not be sent to the patient.", 503)
    ...
    await audit(ctx.session, ctx.user, "APPOINTMENT_RESCHEDULE_REQUESTED", "CareAppointment", row.id, row.branch_id)
    await ctx.session.commit()
    return {**model_dict(row), "booking_url": url, "suggested_start": start, "suggested_end": end}
