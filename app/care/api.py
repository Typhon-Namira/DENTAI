...
from app.care.service import process_inbound_message, settings_for_branch
...
async def get_care_settings(branch_id: uuid.UUID, ctx: Annotated[AuthContext, Depends(current_context)]):
...
async def list_care_plans(
    ...
    result = []
    for plan in plans:
        items = (await ctx.session.scalars(select(CarePlanItem).where(CarePlanItem.care_plan_id == plan.id))).all()
...
async def appointments(
    ...
    query = select(CareAppointment).where(
    ...
    patients = {p.id: p for p in (await ctx.session.scalars(select(Patient).where(Patient.id.in_(patient_ids)))).all()} if patient_ids else {}
    return [{**model_dict(row), "patient": model_dict(patients[row.patient_id]) if row.patient_id in patients else None} for row in rows]
...
async def request_reschedule(
    ...
    if row.conversation_id:
        ctx.session.add(CareConversationMessage(
            ...
            message_metadata={"kind": "doctor_reschedule", "preferred_start": body.preferred_start.isoformat() if body.preferred_start else None},
        ))
    await ctx.session.commit()
    return model_dict(row)
...
async def conversations(ctx: Annotated[AuthContext, Depends(current_context)], limit: int = Query(default=100, ge=1, le=200)):
    query = select(CareConversation).order_by(CareConversation.last_message_at.desc().nullslast(), CareConversation.created_at.desc()).limit(limit)
    if ctx.user.role != Role.DIRECTOR:
    ...
    result = []
    for row in rows:
        appointment = await ctx.session.scalar(select(CareAppointment).where(CareAppointment.conversation_id == row.id).order_by(CareAppointment.created_at.desc()).limit(1))
        result.append({**model_dict(row), "patient": model_dict(patients[row.patient_id]) if row.patient_id in patients else None, "latest_appointment": model_dict(appointment) if appointment else None})
    return result


@router.get("/conversations/{conversation_id}/messages")
async def conversation_messages(conversation_id: uuid.UUID, ctx: Annotated[AuthContext, Depends(current_context)]):
    ...
    rows = (await ctx.session.scalars(select(CareConversationMessage).where(CareConversationMessage.conversation_id == conversation.id).order_by(CareConversationMessage.created_at.asc()))).all()
