...
_ACTIVE_APPOINTMENT_STATES = ["PROPOSED", "APPROVED", "RESCHEDULE_PROPOSED"]
...
def _time_range(item: object) -> tuple[str, str] | None:
    ...
    if isinstance(item, dict):
        dict_start, dict_end = item.get("start"), item.get("end")
        if isinstance(dict_start, str) and isinstance(dict_end, str):
    ...
    for item in settings.blocked_windows or []:
        ...
        try:
            if isinstance(start_raw, str) and isinstance(end_raw, str) and "T" in start_raw:
                ...
                item.get("day") == local_start.weekday()
                ...
                block_start = datetime.combine(
                    local_start.date(), time(h1, m1), tzinfo=local_start.tzinfo
                )
                block_end = datetime.combine(
                    local_start.date(), time(h2, m2), tzinfo=local_start.tzinfo
                )
                if local_start < block_end and local_end > block_start:
                    return True
...
async def settings_for_branch(session: AsyncSession, branch_id: uuid.UUID) -> ClinicCareSettings:
    row = await session.scalar(
        select(ClinicCareSettings).where(ClinicCareSettings.branch_id == branch_id)
    )
    if row:
        return row
    row = ClinicCareSettings(
        branch_id=branch_id, working_days=[0, 1, 2, 3, 4, 5], preferred_times=[], blocked_windows=[]
    )
    session.add(row)
    await session.flush()
    return row
...
async def ensure_care_plan(session: AsyncSession, analysis: AIAnalysis) -> CarePlan | None:
    ...
    language = language_for_phone(
        patient.whatsapp_phone or patient.phone, settings.default_language
    )
    plan = CarePlan(
    ...
    for finding in candidates:
        ...
        session.add(
            CarePlanItem(
                ...
                message_preview=_message(
                    language, patient.first_name, str(finding.tooth_code), finding.finding_type
                ),
...
async def activate_reviewed_plan(
    session: AsyncSession, *, clinic_id: uuid.UUID, clinic_name: str, analysis: AIAnalysis
) -> CarePlan | None:
    ...
    confirmed = [
        item
        for item in items
        if findings.get(item.finding_id) and findings[item.finding_id].review_status == "CONFIRMED"
    ]
    rejected = [
        item
        for item in items
        if findings.get(item.finding_id) and findings[item.finding_id].review_status == "REJECTED"
    ]
    ...
    items = (
        await session.scalars(
            select(CarePlanItem).where(
                ...
                CarePlanItem.status == "FOLLOWUP_READY",
            )
        )
    ).all()
    plan.status = "ACTIVE"
    ...
    occupied = (
        await session.scalars(
            select(CareAppointment).where(
                ...
                CareAppointment.status.in_(_ACTIVE_APPOINTMENT_STATES),
                CareAppointment.starts_at >= local_now.astimezone(UTC),
                CareAppointment.starts_at
...
async def _conversation(
    ...
    row = CareConversation(
        ...
        language=language_for_phone(normalized, plan.language),
        status="ACTIVE",
        booking_context={},
    )
    session.add(row)
...
async def start_or_continue_outreach(
    ...
    *,
    ...
    slots = await available_slots(
        session, branch_id=patient.branch_id, settings=settings, doctor_id=plan.doctor_id, limit=4
    )
    client = WhatsAppServiceClient()
    for index, item in enumerate(pending):
        message = _message(
            conversation.language, patient.first_name, item.tooth_fdi, item.finding_type
        )
        if index == len(pending) - 1 and slots:
            slot_lines = "\n".join(f"• {slot.strftime('%Y-%m-%d %H:%M')}" for slot in slots)
            message += f"\n\n{_slot_heading(conversation.language)}\n{slot_lines}"
        crop = (
            await _finding_crop(session, plan, item)
    ...
    conversation.summary = (
        f"{len(pending)} clinician-confirmed tooth finding(s) sent. Awaiting patient response."
    )
    return conversation
...
async def _find_patient_by_phone(session: AsyncSession, phone: str) -> Patient | None:
    ...
    exact = await session.scalar(
        select(Patient)
        .where((Patient.whatsapp_phone == normalized) | (Patient.phone == normalized))
        .limit(1)
    )
    if exact:
        return exact
    ...
    *,
    ...
    items = (
        ...
        if not plan
            ...
            await session.scalars(
                select(CarePlanItem).where(
                    CarePlanItem.care_plan_id == plan.id, CarePlanItem.status != "REJECTED"
                )
            )
        ).all()
    )
    ...
    slots = await available_slots(
        session, branch_id=patient.branch_id, settings=settings, doctor_id=doctor_id, limit=8
    )
    ...
    reply = await care_agent_reply(
        ...
        clinic_name=clinic_name,
        care_items=[
            {"tooth": x.tooth_fdi, "finding": x.finding_type, "window": x.recommended_window}
            for x in items
        ],
        history=[
            {"role": "assistant" if x.direction == "OUT" else "user", "content": x.body}
    ...
    if reply.selected_slot and reply.selected_slot in slot_strings:
        fresh_slots = await available_slots(
            session, branch_id=patient.branch_id, settings=settings, doctor_id=doctor_id, limit=64
        )
        ...
        if reply.selected_slot in fresh_strings:
            ...
            existing = await session.scalar(
                ...
                .order_by(CareAppointment.starts_at.desc())
            )
    ...
    conversation.last_message_at = datetime.now(UTC)
    conversation.summary = f"Last intent: {reply.intent}. Language: {language_name(conversation.language)}. {'Appointment awaiting doctor approval.' if selected else 'Conversation active.'}"
    return {
        "handled": True,
        "intent": reply.intent,
