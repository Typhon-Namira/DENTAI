...
def market_code(country: str | None, city: str | None = None, address: str | None = None) -> str | None:
    ...
    if any(token in location for token in ("yerevan", "երեւան", "երևան", "armenia", "հայաստան", "ереван", "армения")):
        return "AM"
    if any(token in location for token in ("russia", "россия", "moscow", "москва", "saint petersburg", "санкт петербург")):
        return "RU"
    return None
    ...
    sent_rows = list(
        (
            await session.scalars(
                ...
                .order_by(AccessRequest.payment_instructions_sent_at.asc(), AccessRequest.created_at.asc())
            )
        ).all()
    )
