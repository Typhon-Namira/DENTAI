...
def get_fdi_probs(model, image, box):
    ...
    return torch.softmax(logits.float(), dim=1)[0].detach().cpu()
...
def quadrant_from_probs(probs):
    ...
    for q, classes in QUADRANTS.items():
        q_scores[q] = sum(float(probs[CLASS_TO_IDX[c]]) for c in classes)
...
def sort_quadrant(teeth, q):
    ...
    return sorted(teeth, key=lambda t: center_x(t["bbox"]), reverse=reverse)
...
def sequence_resolve(teeth, q):
    ...
    for i in range(n + 1):
        for j in range(m + 1):
            ...
            if i < n and j < m:
                label = labels[j]
                p = max(float(teeth[i]["probs"][CLASS_TO_IDX[label]]), 1e-8)
    ...
    end_j = min(range(m + 1), key=lambda j: dp[n][j] + (m - j) * missing_penalty)
        ...
        results.append(
            {
                **tooth,
                ...
                "was_changed": (resolved is not None and resolved != tooth["raw"]),
...
def resolve_image(pred_teeth):
    ...
    for tooth in pred_teeth:
        q = quadrant_from_probs(tooth["probs"])
    ...
    for q in ("1", "2", "3", "4"):
        if groups[q]:
            output.extend(sequence_resolve(groups[q], q))

    # V3.1 conservative duplicate cleanup
    from collections import Counter

    for q, expected in QUADRANTS.items():
        group = [t for t in output if str(t["resolved"]).startswith(q)]
        ...
        for duplicate in duplicates:
            ...
            for t in members:
                if t is keeper or not missing:
                    continue

                same_q = sorted(group, key=lambda z: center_x(z["bbox"]), reverse=(q in ("1", "3")))

                rank = same_q.index(t)

                approx_pos = rank * 7.0 / (len(same_q) - 1) if len(same_q) > 1 else 0.0

                candidate = min(missing, key=lambda c: abs((int(c[1]) - 1) - approx_pos))

                t["resolved"] = candidate
                t["was_changed"] = candidate != t["raw"]
                t["v31_cleanup"] = True
...
def main():
    print("=" * 70)
    print("FDI RESOLVER V3.1 GLOBAL TEST")
    print("Device:", DEVICE)
    print("=" * 70)
    ...
    for image_index, record in enumerate(data["records"], start=1):
        image = Image.open(record["image_path"]).convert("RGB")

        image_tensor = to_tensor(image).to(DEVICE)

        with torch.no_grad():
            pred = tooth_model([image_tensor])[0]
        ...
        for box in boxes:
            ...
            probs = get_fdi_probs(fdi_model, image, b)

            conf, idx = probs.max(0)

            raw = FDI_CLASSES[idx.item()]
        ...
        raw_counts = Counter(t["raw"] for t in pred_teeth)

        if any(n > 1 for n in raw_counts.values()):
            raw_duplicate_images += 1

        resolved = resolve_image(pred_teeth)

        v3_counts = Counter(t["resolved"] for t in resolved)

        if any(n > 1 for n in v3_counts.values()):
        ...
        for gt in gt_instances:
            ...
            if p["was_changed"]:
                ...
                if not raw_ok and v3_ok:
                    changed_correctly += 1

                elif raw_ok and not v3_ok:
                    changed_incorrectly += 1

        if image_index % 20 == 0:
            print(f"processed {image_index}/{len(data['records'])}")
    ...
    print()
    print("=" * 70)
    print("FDI RESOLVER V3.1 RESULTS")
    print("=" * 70)

    print("Matched teeth:", matched_teeth)

    print("Raw FDI accuracy:", round(raw_acc, 4))

    print("Resolver V2 reference:", 0.9001)

    print("Resolver V3.1 accuracy:", round(v3_acc, 4))

    print("Absolute improvement vs raw:", round((v3_acc - raw_acc) * 100, 2), "percentage points")

    print("Images with duplicate FDI before:", raw_duplicate_images)

    print("Images with duplicate FDI after V3:", v3_duplicate_images)

    print("Assignments changed:", changed)

    print("Wrong -> correct changes:", changed_correctly)

    print("Correct -> wrong changes:", changed_incorrectly)

    if v3_acc > 0.9001:
        print()
        print("✅ V3 BEATS RESOLVER V2")
    else:
        print()
        print("⚠️ V3 DOES NOT BEAT V2 — do not deploy it yet")
