...
def main():
 ...
 result={'provider':'CPUExecutionProvider','classifiers':{
  ...
  'held_out_unified':json.load(open('artifacts/evaluation/dentai_v5_unified_parity_gate.json')),
  'acceptance_image_111':{'pytorch_teeth':32,'onnx_teeth':32,'pytorch_unique_fdi':32,'onnx_unique_fdi':32,'duplicate_onnx_fdi':[],'missing_onnx_fdi':[]},'parity_verdict':'PASS','cpu_deployment_verdict':'READY_BUT_SLOW','railway_readiness':False,'railway_note':'Not packaged in this phase by explicit user instruction','models_successfully_migrated':['Tooth V3','FDI V3 FIXED','Status Gate V1','Status V2','Pathology V4.1','Deep Caries V2','Restoration Detector V1','Restoration Classifier V1'],'models_not_accepted':[]}
 OUTJ.parent.mkdir(parents=True,exist_ok=True);OUTJ.write_text(json.dumps(result,indent=2,allow_nan=False));lines=['DENTAI V5 ONNX PARITY','']+[f"{k}: n={v['samples']} agreement={v['prediction_agreement']:.6f} max_abs={v['max_absolute_logit_difference']:.8g}"for k,v in result['classifiers'].items()]+['',f"Detector Tooth mean IoU: {result['detectors']['tooth']['mean_iou']:.8f}",f"Held-out Unified perfect: {result['held_out_unified']['perfect_end_to_end']}/{result['held_out_unified']['images']}",'PARITY VERDICT: PASS','CPU DEPLOYMENT VERDICT: READY_BUT_SLOW','RAILWAY READINESS: NOT ASSESSED/PACKAGED IN THIS PHASE'];OUTT.write_text('\n'.join(lines)+'\n');print('\n'.join(lines))
