...
def json_safe(value):
    if isinstance(value, torch.Tensor):
        return (
            ...
            if value.numel() <= 100
            else {"tensor_shape": list(value.shape)}
        )
    if isinstance(value, Path):
        return str(value)
    if isinstance(value, dict):
        return {str(k): json_safe(v) for k, v in value.items() if k != "model"}
...
def load_strict(model, path):
    ...
    state = checkpoint.get(
        "model", checkpoint.get("model_state_dict", checkpoint.get("state_dict", checkpoint))
    )
...
def classifier_models():
    ...
    restoration = models.resnet18(weights=None)
    restoration.fc = nn.Sequential(nn.Dropout(0.30), nn.Linear(restoration.fc.in_features, 2))
    specs = {
        "fdi": (fdi, v5.CHECKPOINTS["fdi"], "fdi_v3.onnx", ["image", "spatial"], ["logits"]),
        "status_gate": (
            gate,
            v5.CHECKPOINTS["status_gate"],
            "status_gate_v1.onnx",
            ["image"],
            ["logits"],
        ),
        ...
        "deep_caries": (
            deep,
            v5.CHECKPOINTS["deep_caries"],
            "deep_caries_v2.onnx",
            ["image"],
            ["logits"],
        ),
        "restoration_classifier": (
            restoration,
            v5.CHECKPOINTS["restoration_classifier"],
            "restoration_classifier_v1.onnx",
            ["image"],
            ["logits"],
        ),
    }
    return {
        key: (load_strict(model, path)[0], path, filename, inputs, outputs)
        for key, (model, path, filename, inputs, outputs) in specs.items()
    }


def real_inputs():
    status_record = json.loads(Path("data/canonical/dual_labeled_status/test.json").read_text())[
        "records"
    ][0]
    ...
    rest_record = next(
        (r, x)
        for r in rest_data["images"]
        for x in r.get("instances", [])
        if str(x.get("canonical_class", "")).upper() in ("FILLING", "IMPLANT")
        and x.get("bbox_xyxy")
    )
    rest_image = Image.open(
        Path("data/raw/akudental/current/source_repo/AKUDENTAL/images")
        / str(rest_record[0]["source_image_id"])
    ).convert("RGB")
    fdi_tensor = v5.crop_tensor(status_image, status_box, 0.35, 12, 224).cpu()
    width, height = status_image.size
    x1, y1, x2, y2 = map(float, status_box)
    ...
    return {
        ...
        "status_gate": (v5.crop_tensor(status_image, status_box, 0.35, 16, 224).cpu(),),
        "status_v2": (v5.crop_tensor(status_image, status_box, 0.45, 18, 256).cpu(),),
        "deep_caries": (
            v5.crop_tensor(deep_image, deep_record[1]["bbox_xyxy"], 0.55, 24, 256).cpu(),
        ),
        "restoration_classifier": (
            v5.crop_tensor(rest_image, rest_record[1]["bbox_xyxy"], 0.45, 15, 224).cpu(),
        ),
    }
...
def export_classifiers():
    ...
    for key, (model, _, filename, input_names, output_names) in classifier_models().items():
        ...
        if not agreement or float(abs_diff.max()) > 1e-3:
            raise RuntimeError(
                f"{key} parity failed: agreement={agreement}, max_abs={abs_diff.max()}"
            )
        results[key] = {
            "onnx_path": str(target),
            "onnx_sha256": sha256(target),
            "max_abs_logit_difference": float(abs_diff.max()),
            "mean_abs_logit_difference": float(abs_diff.mean()),
...
def detector_models():
    ...
    return {
        ...
        "pathology": (
            load_strict(pathology, v5.CHECKPOINTS["pathology"])[0],
            "pathology_v41.onnx",
            (640, 1312),
        ),
        "restoration_detector": (
            load_strict(restoration, v5.CHECKPOINTS["restoration_detector"])[0],
            "restoration_detector_v1.onnx",
            (650, 1333),
        ),
    }
...
def export_detectors():
    ...
    results = {}
    selected = next(
        (sys.argv[i + 1] for i, x in enumerate(sys.argv[:-1]) if x == "--detector"), None
    )
    for key, (model, filename, shape) in detector_models().items():
        if selected and key != selected:
            continue
        wrapper = DetectionOutputWrapper(model).eval()
        target = ONNX_DIR / filename
        ...
        results[key] = {
            "onnx_path": str(target),
            "onnx_sha256": sha256(target),
            "fixed_input_shape": list(example.shape),
...
def export_resize_preprocessor():
    ...
    output = helper.make_tensor_value_info(
        "resized", TensorProto.FLOAT, [1, 3, "new_height", "new_width"]
    )
    roi = helper.make_tensor("roi", TensorProto.FLOAT, [0], [])
    scales = helper.make_tensor("scales", TensorProto.FLOAT, [0], [])
    node = helper.make_node(
        "Resize",
        ["image", "roi", "scales", "sizes"],
        ["resized"],
        mode="linear",
        coordinate_transformation_mode="half_pixel",
    )
    graph = helper.make_graph(
        [node], "dentai_detector_resize", [image, sizes], [output], [roi, scales]
    )
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", OPSET)])
    model.ir_version = 10
    target = ONNX_DIR / "detector_preprocess.onnx"
...
def create_manifest(exports=None):
    architectures = {
        "tooth": (
            "Mask R-CNN ResNet-50 FPN; 2 classes; min_size=640 max_size=1600",
            "variable RGB panorama",
            "boxes, labels, scores (masks unused downstream)",
            ["BACKGROUND", "TOOTH"],
        ),
        "fdi": (
            "FDINetV2: ResNet-18 visual backbone + 4D spatial MLP",
            "N x 3 x 224 x 224 plus N x 4",
            "32-class FDI logits",
            v5.FDI_CLASSES,
        ),
        "status_gate": (
            "ResNet-18; Dropout(0.30)+Linear(2)",
            "N x 3 x 224 x 224",
            "HEALTHY/NON_HEALTHY logits",
            v5.STATUS_GATE_CLASSES,
        ),
        "status_v2": (
            "ResNet-34; Dropout(0.35)+Linear(7)",
            "N x 3 x 256 x 256",
            "7-class status logits",
            v5.STATUS_CLASSES,
        ),
        "pathology": (
            "Faster R-CNN ResNet-50 FPN; 7 classes; min_size=640 max_size=1600",
            "variable RGB panorama",
            "boxes, labels, scores",
            ["BACKGROUND", *v5.PATHOLOGY_THRESHOLDS],
        ),
        "deep_caries": (
            "ResNet-34; Dropout(0.40)+Linear(2)",
            "N x 3 x 256 x 256",
            "CARIES/DEEP_CARIES logits",
            v5.DEEP_CARIES_CLASSES,
        ),
        "restoration_detector": (
            "Faster R-CNN ResNet-50 FPN; 3 classes",
            "variable RGB panorama",
            "boxes, labels, scores",
            ["BACKGROUND", "FILLING", "IMPLANT"],
        ),
        "restoration_classifier": (
            "ResNet-18; Dropout(0.30)+Linear(2)",
            "N x 3 x 224 x 224",
            "FILLING/IMPLANT logits",
            ["FILLING", "IMPLANT"],
        ),
    }
    preprocessing = {
    ...
    for key, path in v5.CHECKPOINTS.items():
        ...
        models_manifest[key] = {
            ...
            "checkpoint_metric_metadata": json_safe(ckpt),
            "onnx_export": (exports or {}).get(key),
        }
    manifest = {
        "model_version": "dentai-unified-v5",
        "freeze_status": "PRODUCTION_FROZEN",
        "reference_engine": "ai_engine/inference/dentai_unified_v5.py",
        "opset": OPSET,
        "thresholds": {
            "tooth": 0.5,
    ...
    if "--detectors" in sys.argv:
        ...
        if existing:
            for key, value in exports.items():
                existing["models"][key]["onnx_export"] = value
            MANIFEST_PATH.write_text(
                json.dumps(existing, indent=2, allow_nan=False), encoding="utf-8"
            )
