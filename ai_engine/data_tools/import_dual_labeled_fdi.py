...
VALID_FDI = {str(q * 10 + n) for q in range(1, 5) for n in range(1, 9)}

image_map = {p.stem: p for p in IMAGES.rglob("*") if p.is_file() and p.suffix.lower() in IMAGE_EXTS}
...
for label_path in sorted(LABELS.glob("*.json")):
    ...
    record_id = hashlib.sha1(str(image_path).encode("utf-8")).hexdigest()[:16]
...
output = {
    ...
    "task": ["tooth_instance_segmentation", "fdi_identification"],
...
stats_output = {
    ...
    "fdi_counts": dict(sorted(fdi_stats.items(), key=lambda x: int(x[0]))),
}
