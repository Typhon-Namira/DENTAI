...
image_map = {p.stem: p for p in IMAGES.iterdir() if p.is_file()}
...
for p in sorted(LABELS.glob("*.json")):
        ...
        teeth.append(
            {
                "fdi_number": fdi,
                "status": status,
                "status_source_value": (0 if gid is None else gid),
                "bbox_xyxy": bbox,
                "polygon": points,
            }
        )
    ...
    records.append(
        {
            "source_dataset": "dual_labeled_status",
            "source_image_id": image_map[p.stem].name,
            "image_path": str(image_map[p.stem]),
...
for split, recs in splits.items():
    payload = {
        "schema_version": "dual-labeled-status-v1",
        "split": split,
        "status_mapping": {
            "0": "HEALTHY",
            "1": "FILLING",
