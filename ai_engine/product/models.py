...
class IdentityStatus(str, Enum):
    ...
    IDENTITY_NEEDS_CONFIRMATION = "IDENTITY_NEEDS_CONFIRMATION"
...
class ProductIdentityState(str, Enum):
    ...
    IDENTITY_MATCHED_EXISTING = "IDENTITY_MATCHED_EXISTING"
    IDENTITY_NEW_PATIENT_CANDIDATE = "IDENTITY_NEW_PATIENT_CANDIDATE"
...
class ReviewAction(str, Enum):
    ...
    MODIFY_FDI = "MODIFY_FDI"


class LabelState(str, Enum):
    UNLABELED = "UNLABELED"
    AI_PRELABELED = "AI_PRELABELED"
...
class ChangeState(str, Enum):
    STABLE = "STABLE"
    STABLE_WITH_CONFIDENCE_CHANGE = "STABLE_WITH_CONFIDENCE_CHANGE"
    IMPROVED = "IMPROVED"
    PROGRESSED = "PROGRESSED"
    NEW_FINDING = "NEW_FINDING"
    RESOLVED = "RESOLVED"
    UNKNOWN = "UNKNOWN"
    CHANGE_UNCERTAIN = "CHANGE_UNCERTAIN"
...
class ExtractedIdentity(StrictModel):
    ...
    sex: ExtractedField | None = None
    study_date: ExtractedField | None = None
    clinic_name: ExtractedField | None = None
...
class Patient(StrictModel):
    patient_id: str
    clinic_id: str
    clinic_patient_number: str | None = None
...
class PatientMatch(StrictModel):
    patient_id: str
...
class IdentityResolution(StrictModel):
    ...
    candidate_patient_matches: list[PatientMatch] = Field(default_factory=list)
    requires_confirmation: bool = True
    confidence: float = 0
    conflict_reasons: list[str] = Field(default_factory=list)


class FollowupWindow(StrictModel):
...
class FollowupRecommendation(StrictModel):
    ...
    recommended_followup_start: date
    recommended_followup_end: date
    target_followup_date: date
    reasons: list[str]
    ...
    status: str = "OPEN"
    doctor_overridden: bool = False
    disclaimer: str = (
        "Product-configured reassessment timing; not a predicted disease occurrence date."
    )
...
class ImmutableAnalysis(StrictModel):
...
class DoctorReview(StrictModel):
    ...
    doctor_action: ReviewAction
    corrected_value: Any = None
    notes: str | None = None


class LearningRecord(StrictModel):
    learning_record_id: str
    clinic_id: str
    source_case_id: str
    source_model_version: str
    verification_status: LabelState
    deidentification_status: str
    training_eligibility: bool = False
    consent_or_policy_reference: str | None = None
    deidentified_asset_reference: str | None = None
    original_asset_reference: str | None = None
    original_ai_output_reference: str | None = None
...
class PatientProfile(StrictModel):
    ...
    identity_state: ProductIdentityState = ProductIdentityState.IDENTITY_FOUND
    created_at: datetime = Field(default_factory=utcnow)
    updated_at: datetime = Field(default_factory=utcnow)
...
class IdentityCandidate(StrictModel):
    ...
    clinic_id: str
    study_id: str
    state: ProductIdentityState
    extracted_identity: ExtractedIdentity
    candidate_matches: list[PatientMatch] = Field(default_factory=list)
...
class StudyRecord(StrictModel):
    ...
    clinic_id: str
    patient_id: str | None = None
    original_image_reference: dict[str, Any]
    study_date: date | None = None
    identity_state: ProductIdentityState
...
class DentaiCaseAnalysis(StrictModel):
    case_id: str
    clinic_id: str
    ...
    longitudinal_comparison: dict[str, Any] | None
