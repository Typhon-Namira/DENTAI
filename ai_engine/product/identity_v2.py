...
class OCRIdentityExtractor(IdentityExtractor):
    ...
    def extract(self, path: Path) -> IdentityExtractionResult:
        raw = self.provider.extract(path)
        fields = {}
        raw_text = None
        ...
        return IdentityExtractionResult(
            fields=fields, raw_text=raw_text, provider=type(self.provider).__name__
        )
...
class DICOMIdentityExtractor(IdentityExtractor):
    def extract(self, path: Path) -> IdentityExtractionResult:
        try:
            ...
            return IdentityExtractionResult(fields={}, raw_text=None, provider="DICOM_UNAVAILABLE")
        ...
        for tag, name in DICOM_FIELDS.items():
            ...
            fields[key] = ExtractedField(
                value=value.replace("^", " ") if tag == "PatientName" else value,
                source="DICOM",
                confidence=1.0,
                verified=False,
                normalized_value=normalize_text(value),
...
def extracted_identity(result: IdentityExtractionResult) -> ExtractedIdentity:
    known = {
        k: v for k, v in result.get("fields", {}).items() if k in ExtractedIdentity.model_fields
    }
    metadata = {
        k: v for k, v in result.get("fields", {}).items() if k not in ExtractedIdentity.model_fields
    }
    return ExtractedIdentity(**known, metadata=metadata)


def decide_identity_state(
    identity: ExtractedIdentity, matches: list[PatientMatch], *, minimum_confidence: float = 0.75
) -> ProductIdentityState:
    evidence = [
        x
        for x in (identity.patient_id, identity.patient_name, identity.date_of_birth)
        if x and x.value
    ]
    if not evidence:
        return ProductIdentityState.IDENTITY_NOT_FOUND
    if len(matches) > 1 and matches[0].match_score - matches[1].match_score < 0.10:
        return ProductIdentityState.IDENTITY_REVIEW_REQUIRED
    if (
        matches
        and matches[0].match_score >= 0.95
        and any(
            r in matches[0].match_reasons
            for r in (
                "EXACT_CLINIC_PATIENT_ID",
                "EXACT_DICOM_PATIENT_ID",
                "EXACT_EXTERNAL_PATIENT_ID",
            )
        )
    ):
        return ProductIdentityState.IDENTITY_MATCHED_EXISTING
    if max(x.confidence for x in evidence) < minimum_confidence:
        return ProductIdentityState.IDENTITY_REVIEW_REQUIRED
    if identity.patient_name and not identity.patient_id:
        return ProductIdentityState.IDENTITY_REVIEW_REQUIRED
...
def profile_to_patient(profile: PatientProfile):
    ...
    return Patient(
        ...
        clinic_patient_number=profile.clinic_local_identifier,
