...
class TesseractOCRProvider(IdentityOCRProvider):
    ...
    def extract(self, image_path: Path):
        ...
        return {"raw_text": (text, 0.50)} if text.strip() else {}
...
def extract_dicom_identity(path: Path) -> ExtractedIdentity:
    ...
    for tag, name in DICOM_FIELDS.items():
        ...
        if name in ExtractedIdentity.model_fields:
            values[name] = field
        else:
            meta[name] = field
...
def extract_image_identity(
    path: Path, provider: IdentityOCRProvider | None = None
) -> ExtractedIdentity:
    raw = (provider or NoOpOCRProvider()).extract(path)
    values: dict[str, object] = {}
    meta = {}
...
def extract_identity(
    path: str | Path, provider: IdentityOCRProvider | None = None
) -> IdentityResolution:
    p = Path(path)
    identity = (
        extract_dicom_identity(p)
        if p.suffix.lower() in (".dcm", ".dicom")
        else extract_image_identity(p, provider)
    )
    found = any(
        getattr(identity, x) and getattr(identity, x).value
        for x in ("patient_name", "patient_id", "date_of_birth")
    )
    return IdentityResolution(
        status=IdentityStatus.IDENTITY_NEEDS_CONFIRMATION
        if found
        else IdentityStatus.IDENTITY_NOT_FOUND,
        extracted_identity=identity,
        requires_confirmation=True,
        confidence=max(
            [
                getattr(identity, x).confidence
                for x in ("patient_name", "patient_id", "date_of_birth")
                if getattr(identity, x)
            ],
            default=0,
        ),
    )
