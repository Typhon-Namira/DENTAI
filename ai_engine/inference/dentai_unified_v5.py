...
def load_models():
    ...
    return {
        ...
        "pathology": pathology,
        "deep_caries": deep,
        "restoration_detector": rest_det,
        "restoration_classifier": rest_cls,
    }, {
        "tooth": tooth_ckpt,
        "fdi": fdi_ckpt,
        "status_gate": gate_ckpt,
        "status_v2": status_ckpt,
        "pathology": pathology_ckpt,
        "deep_caries": deep_ckpt,
        "restoration_detector": rest_det_ckpt,
...
def crop_tensor(image, box, padding, minimum, size):
    ...
    tf = transforms.Compose(
        [transforms.Resize((size, size)), transforms.ToTensor(), IMAGENET_NORMALIZE]
    )
...
def classify(model, tensor, classes):
    ...
    return (
        classes[int(idx)],
        float(conf),
        {c: round(float(p), 6) for c, p in zip(classes, probs.cpu())},
    )
...
def fdi_probs(model, image, box):
    ...
    spatial = torch.tensor(
        [[(x1 + x2) / 2 / width, (y1 + y2) / 2 / height, bw / width, bh / height]], device=DEVICE
    )
        ...
        results.append(
            {
                ...
                "type_agreement": det_type == cls_type,
            }
        )
    return results
...
def fuse_tooth(tooth, image, deep_model):
    ...
    findings = (
        []
        if status == "HEALTHY"
        else (["CROWN", "ROOT_CANAL_TREATMENT"] if status == "RCT_CROWN" else [status])
    )
    ...
    for rest in tooth["restorations"]:
        ...
        if (
            status == "FILLING"
            and rest["classifier_type"] == "IMPLANT"
            and rest["classifier_confidence"] >= 0.70
        ):
    ...
    if has_caries:
        label, confidence, probabilities = classify(
            deep_model,
            crop_tensor(image, tooth["tooth_detection"]["bbox_xyxy"], 0.55, 24, 256),
            DEEP_CARIES_CLASSES,
        )
...
def checkpoint_metadata(ckpts):
    names = {
        "tooth": "Tooth V3",
        "fdi": "FDI V3 FIXED",
        "status_gate": "Tooth Status Gate V1",
        "status_v2": "Tooth Status V2",
        "pathology": "Pathology Detector V4.1",
        "deep_caries": "Deep Caries Specialist V2",
        "restoration_detector": "Restoration Detector V1",
...
def save_preview(image, teeth, path):
    ...
    for tooth in teeth:
        ...
        draw.rectangle(
            (x1, y1, x2, y2), outline=(255, 210, 0) if review else (0, 255, 100), width=2
        )
...
def run(image_path, tooth_threshold=0.50, restoration_threshold=0.50):
    ...
    for index, (box, score) in enumerate(zip(output["boxes"].cpu(), output["scores"].cpu()), 1):
        ...
        conf, idx = probs.max(0)
        teeth_for_resolver.append(
            {
                "instance_id": index,
                "bbox": bbox,
                "probs": probs,
                "raw": FDI_CLASSES[int(idx)],
                "raw_conf": float(conf),
    ...
    for item in resolved:
        bbox = item["bbox"]
        gate_pred, gate_conf, gate_probs = classify(
            models_by_name["status_gate"],
            crop_tensor(image, bbox, 0.35, 16, 224),
            STATUS_GATE_CLASSES,
        )
        nonhealthy = gate_probs["NON_HEALTHY"]
        gate_effective = "NON_HEALTHY" if nonhealthy >= 0.30 else "HEALTHY"
        status_pred, status_conf, status_probs = classify(
            models_by_name["status_v2"], crop_tensor(image, bbox, 0.45, 18, 256), STATUS_CLASSES
        )
        teeth.append(
            {
                "tooth_detection": {
                ...
                "fdi": item["resolved"],
                "fdi_confidence": round(item["raw_conf"], 6),
                "raw_fdi": item["raw"],
                "fdi_was_changed": bool(item["was_changed"]),
                "duplicate_cleanup_applied": bool(item.get("duplicate_cleanup", False)),
                "fdi_review_required": bool(item["unresolved_by_dp"] or item["raw_conf"] < 0.70),
                "status_gate": {
                    "prediction": gate_pred,
                    "effective_prediction": gate_effective,
                    "confidence": round(gate_conf, 6),
                    "probabilities": gate_probs,
                    "non_healthy_probability": nonhealthy,
                    "abnormal_threshold": 0.30,
                },
                "status_v2": {
                    "prediction": status_pred,
                    "confidence": round(status_conf, 6),
                    "probabilities": status_probs,
                },
            }
        )
    del output, teeth_for_resolver, resolved
    pathology = run_detector(
        models_by_name["pathology"], image, PATHOLOGY_CLASSES, PATHOLOGY_THRESHOLDS
    )
    unmatched_pathologies = attach_detections(pathology, teeth, "pathology_evidence")
    restorations = run_restorations(
    ...
    duplicate_fdis = sorted(
        fdi for fdi, count in Counter(t["fdi"] for t in teeth).items() if count > 1
    )
    result = {
        "version": "dentai-unified-v5",
        "image": str(image_path),
        "device": str(DEVICE),
        "models": checkpoint_metadata(ckpts),
        "thresholds": {
            "tooth": tooth_threshold,
            "status_gate_non_healthy": 0.30,
            "pathology": PATHOLOGY_THRESHOLDS,
            "deep_caries": 0.65,
            "restoration": restoration_threshold,
        },
        "summary": {
            "teeth": len(teeth),
            "unique_fdi": len(set(t["fdi"] for t in teeth)),
            "duplicate_fdi": duplicate_fdis,
            "pathology_detections": len(pathology),
            "restorations": len(restorations),
            "review_required": sum(t["review_required"] for t in teeth),
            "resolver_duplicate_cleanup_changes": cleanup,
        },
        "teeth": teeth,
        "unmatched_pathologies": unmatched_pathologies,
        "unmatched_restorations": unmatched_restorations,
    }
    out = Path("artifacts/unified")
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "dentai_unified_v5.json"
    preview_path = out / "dentai_unified_v5_preview.jpg"
    ...
    print("=" * 60)
    print("DENTAI UNIFIED BRAIN V5")
    print("=" * 60)
    for key, title in [("tooth", "Tooth model"), ("fdi", "FDI model")]:
        print(f"{title}: {labels[key]['name']}")
    ...
    for key, title in [
        ...
        ("restoration_detector", "Restoration Detector"),
        ("restoration_classifier", "Restoration Classifier"),
    ]:
        print(f"{title}: {labels[key]['name']}")
    print()
    print("Teeth:", len(teeth))
    print("Unique FDI:", len(set(t["fdi"] for t in teeth)))
    print("Pathology detections:", len(pathology))
    print("Restorations:", len(restorations))
    print("Review required:", sum(t["review_required"] for t in teeth))
    print("JSON:", json_path)
    print("Preview:", preview_path)
    print("=" * 60)
    for tooth in teeth:
        print(
            f"FDI {tooth['fdi']} | STATUS={tooth['status_v2']['prediction']} | FINDINGS={','.join(tooth['final_findings'])} | REVIEW={tooth['review_required']}"
        )
    return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tooth-threshold", type=float, default=0.50)
    parser.add_argument("--restoration-threshold", type=float, default=0.50)
