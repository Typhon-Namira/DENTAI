...
def load_fdi():
    p = Path("checkpoints/fdi_v2_final/fdi_v2_best_90_38.pt")

    if not p.exists():
        p = Path("checkpoints/fdi_v2/best.pt")
    ...
    print("✓ FDI V2 | val_acc=", ckpt.get("val_acc"))
...
def load_restoration_gate():
    p = Path("checkpoints/restoration_gate_v1/best.pt")
    ...
    print("✓ Restoration Gate V1 | test_acc≈0.951")
...
def geometry(box, W, H):
    ...
    return (x1, y1, x2, y2, bw, bh, cx, cy, bw / max(W, 1), bh / max(H, 1))


def crop_with_padding(image, box, px, py):
    W, H = image.size

    x1, y1, x2, y2, bw, bh, *_ = geometry(box, W, H)
...
def infer_fdi(model, image, box):
    ...
    (x1, y1, x2, y2, bw, bh, cx, cy, nw, nh) = geometry(box, W, H)

    spatial = torch.tensor([[cx, cy, nw, nh]], dtype=torch.float32, device=DEVICE)

    crop = image.crop(
        (
            max(0, int(x1) - max(12, int(bw * 0.35))),
            max(0, int(y1) - max(12, int(bh * 0.35))),
            min(W, int(x2) + max(12, int(bw * 0.35))),
            min(H, int(y2) + max(12, int(bh * 0.35))),
        )
    )
...
def resolve_arch(teeth):
    ...
    for q, group in groups.items():
        counts = Counter(x["raw_fdi_number"] for x in group)

        duplicates = [x for x, n in counts.items() if n > 1]

        expected = QUADRANTS[q]

        missing = [x for x in expected if x not in counts]

        reverse = q in ("1", "3")

        ordered = sorted(group, key=center_x, reverse=reverse)

        rank = {id(t): i for i, t in enumerate(ordered)}

        for dup in duplicates:
            members = [t for t in group if t["raw_fdi_number"] == dup]

            keeper = max(members, key=lambda t: float(t["fdi_confidence"]))
            ...
            for t in members:
                ...
                approx = r * 7 / (len(ordered) - 1) if len(ordered) > 1 else 0

                candidate = min(missing, key=lambda c: abs((int(c[1]) - 1) - approx))
...
def disease_meta(image, box, fdi):
    W, H = image.size
    ...
    meta = torch.tensor(
        [[cx, cy, nw, nh, fdi_idx, quadrant, position]], dtype=torch.float32, device=DEVICE
    )
    ...
    return (DISEASE_TF(crop).unsqueeze(0).to(DEVICE), meta)


def infer_disease(a, b, image, box, fdi):
    tensor, meta = disease_meta(image, box, fdi)
    ...
    stage_a = STAGE_A_CLASSES[ia.item()]
    ...
    if stage_a == "Decay":
        ...
        return {"candidate": STAGE_B_CLASSES[ib.item()], "confidence": float(ca.item() * cb.item())}

    return {"candidate": stage_a, "confidence": float(ca.item())}


def infer_restoration_gate(model, image, box):
    crop = crop_with_padding(image, box, 0.35, 0.35)

    tensor = REST_TF(crop).unsqueeze(0).to(DEVICE)
    ...
    present_prob = float(probs[0, 1].item())

    absent_prob = float(probs[0, 0].item())

    present = present_prob >= absent_prob

    return {
        "present": present,
        "present_probability": round(present_prob, 4),
        "absent_probability": round(absent_prob, 4),
        # IMPORTANT:
        # type classifier requires localized
        # restoration bbox, so do not invent
        # Filling/Implant here.
        "type": "UNRESOLVED" if present else None,
    }
...
def run(image_path, threshold=0.50):
    ...
    for i, (box, score) in enumerate(zip(boxes, scores), start=1):
        ...
        fdi, fdi_conf = infer_fdi(fdi_model, image, box)
    ...
    for t in teeth:
        resolved = t["resolved_fdi_number"]

        disease = infer_disease(disease_a, disease_b, image, t["bbox_xyxy"], resolved)

        restoration = infer_restoration_gate(rest_gate, image, t["bbox_xyxy"])

        t["disease"] = {
            "candidate": disease["candidate"],
            "confidence": round(disease["confidence"], 4),
            "review_required": True,
            "limitation": "No HEALTHY disease class yet.",
        }

        t["restoration"] = restoration

        t["fdi_review_required"] = t["fdi_confidence"] < 0.70

    teeth.sort(key=lambda t: int(t["resolved_fdi_number"]))

    result = {
        "schema_version": "dentai-unified-v2",
        "image": str(image_path),
        "detected_teeth": len(teeth),
        "models": {
            "tooth": "Tooth V2",
            "fdi": "FDI V2 + Arch Resolver V2",
            "disease": "Disease V3 Hierarchical",
            "restoration_gate": "Restoration Gate V1",
        },
        "limitations": [
    ...
    for t in teeth:
        ...
        fdi = t["resolved_fdi_number"]

        disease = t["disease"]["candidate"]

        rest = "REST" if t["restoration"]["present"] else ""

        text = f"{fdi} {disease} {rest}"

        draw.rectangle([x1, y1, x2, y2], width=3)

        draw.text((x1, max(0, y1 - 15)), text)

    pp = out / "dentai_unified_v2_preview.jpg"

    preview.save(pp, quality=95)

    print()
    print("=" * 60)
    print("DENTAI UNIFIED V2 COMPLETE")
