...
def run(
    ...
    # Resolver V3.1:
    # conservatively clean only duplicates that remain after DP.
    ...
    for q, expected in quadrants.items():
        group = [t for t in resolved if str(t["resolved"]).startswith(q)]
        ...
        for duplicate in duplicates:
            ...
            # Preserve the most confident member.
            ...
            for t in members:
                ...
                # Use spatial position only among missing labels
                ...
                same_q = sorted(
                    group,
                    key=lambda z: (z["bbox"][0] + z["bbox"][2]) / 2.0,
                    reverse=(q in ("1", "3")),
                )
