...
def main():
    ...
    # V4.1 pathology-aware image sampling.
    # Minority / difficult findings are intentionally seen more often.
    CLASS_SAMPLE_WEIGHT = {
        "CARIES": 1.0,
        "APICAL_PERIODONTITIS": 1.4,
        "IMPACTED": 2.0,
        "BONE_RESORPTION": 3.5,
        "ROOT_FRAGMENT": 2.0,
        "FURCATION_LESION": 6.0,
    }
    ...
    for r in train_ds.records:
        ...
        if not present:
            ...
            # max prevents multi-label images from exploding in weight
            w = max(CLASS_SAMPLE_WEIGHT.get(c, 1.0) for c in present)
    ...
    warm = Path("checkpoints/pathology_detector_v4/latest.pt")
    ...
    out = Path("checkpoints/pathology_detector_v41")
    ...
    print()
    print("=" * 72)
    print("PATHOLOGY DETECTOR V4.1 COMPLETE")
