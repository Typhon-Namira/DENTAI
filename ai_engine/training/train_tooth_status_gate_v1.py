    ...
    def __init__(self, split, train=False):
        ...
        for r in d["records"]:
            for t in r["teeth"]:
                ...
                gate = "HEALTHY" if status == "HEALTHY" else "NON_HEALTHY"
...
def evaluate(model, loader, device):
    ...
    per_class = {CLASSES[i]: cc[i] / ct[i] if ct[i] else 0 for i in range(2)}
...
def main():
    ...
    train_ds = StatusGateDataset("train", train=True)

    val_ds = StatusGateDataset("validation", train=False)

    counts = Counter(x["class_name"] for x in train_ds.samples)
    ...
    class_weights = torch.tensor([1.0, 1.6], dtype=torch.float32, device=device)
    ...
    out = Path("checkpoints/tooth_status_gate_v1")
    ...
    print("MODEL:", "checkpoints/tooth_status_gate_v1/best.pt")
    print("=" * 60)
