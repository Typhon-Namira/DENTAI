...
class DiseaseDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        meta = torch.tensor([cx, cy, nw, nh, fdi_idx, quadrant, tooth_pos], dtype=torch.float32)
...
class DiseaseNetV2(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(dim + 64, 256), nn.ReLU(), nn.Dropout(0.40), nn.Linear(256, len(CLASSES))
        )

    def forward(self, image, meta):
        visual = self.backbone(image)
        meta = self.meta(meta)
        return self.classifier(torch.cat([visual, meta], dim=1))
...
def main():
    ...
    train_ds = DiseaseDatasetV2("data/splits/tooth_v2/train.json", train=True)

    val_ds = DiseaseDatasetV2("data/splits/tooth_v2/validation.json", train=False)

    counts = Counter(x["disease"] for x in train_ds.samples)
    ...
    weights = torch.tensor([1.0, 1.35, 1.10, 1.65], dtype=torch.float32, device=device)

    loss_fn = nn.CrossEntropyLoss(weight=weights, label_smoothing=0.06)

    optimizer = torch.optim.AdamW(
        [
            {"params": model.backbone.parameters(), "lr": 4e-5},
    ...
    for epoch in range(14):
        ...
        val_acc, macro, per_class, conf = evaluate(model, val_loader, device)
        ...
        if score > best_score:
            ...
            print(f"*** NEW BEST score={score:.4f} val={val_acc:.4f} macro={macro:.4f} ***")
        else:
            stale += 1

        if stale >= patience:
            print(f"EARLY STOPPING | best_score={best_score:.4f}")
            break
