...
def evaluate(
    ...
    deep_f1 = 2 * deep_precision * deep_recall / max(deep_precision + deep_recall, 1e-8)

    balanced_acc = (deep_recall + caries_recall) / 2
...
def main():
    ...
    train_ds = DeepCariesDataset("train", train=True)

    val_ds = DeepCariesDataset("validation", train=False)

    counts = Counter(s["disease"] for s in train_ds.samples)
    ...
    model.fc = nn.Sequential(nn.Dropout(0.40), nn.Linear(model.fc.in_features, 2))

    model.to(device)

    # Additional loss weighting.
    alpha = torch.tensor([1.0, 1.8], dtype=torch.float32, device=device)
    ...
    out = Path("checkpoints/deep_caries_v2")
    ...
    for epoch in range(12):
        ...
        metrics = evaluate(model, val_loader, device, threshold=0.50)
        ...
        print(f"  CARIES recall: {metrics['caries_recall']:.4f}")

        print(f"  DEEP recall: {metrics['deep_recall']:.4f}")

        print(f"  DEEP precision: {metrics['deep_precision']:.4f}")

        print(f"  DEEP F1: {metrics['deep_f1']:.4f}")
    ...
    print("MODEL:", "checkpoints/deep_caries_v2/best.pt")
    print("=" * 70)
