...
class StatusDataset(Dataset):
    def __init__(self, split, train=False):
        ...
        data = json.loads((ROOT / f"{split}.json").read_text(encoding="utf-8"))
            ...
            self.tf = transforms.Compose(
                [
                    ...
                    transforms.RandomAffine(
                        degrees=0, translate=(0.025, 0.025), scale=(0.94, 1.06)
                    ),
                    transforms.ColorJitter(brightness=0.05, contrast=0.10),
                    transforms.ToTensor(),
...
def main():
    ...
    counts = Counter(s["status"] for s in train_ds.samples)
    ...
    for s in train_ds.samples:
        ...
        sample_weights.append((len(train_ds) / max(n, 1)) ** 0.60)
    ...
    for c in CLASSES:
        weights.append(math.sqrt(max_count / max(counts[c], 1)))

    class_weights = torch.tensor(weights, dtype=torch.float32, device=device)

    loss_fn = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.04)
    ...
    for epoch in range(14):
        ...
        val_acc, macro, recall, cm = evaluate(model, val_loader, device)
    ...
    print("MODEL:", "checkpoints/tooth_status_v2/best.pt")
    print("=" * 70)
