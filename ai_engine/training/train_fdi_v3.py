        ...
        if train:
            self.tf = transforms.Compose(
                [
                    ...
                    transforms.ColorJitter(brightness=0.04, contrast=0.08),
                    transforms.ToTensor(),
...
def main():
    ...
    train_ds = FDIDataset("train", train=True)

    val_ds = FDIDataset("validation", train=False)
    ...
    for s in train_ds.samples:
        ...
        sample_weights.append((max_count / max(n, 1)) ** 0.35)
    ...
    for epoch in range(6):
        ...
        val_acc, macro, recalls = evaluate(model, val_loader, device)
