...
OLD_CKPT = Path("checkpoints/tooth_v2/maskrcnn_fpn_v1/best.pt")

OUT = Path("checkpoints/tooth_v3/maskrcnn_fpn_v1")
...
def normalize_polygon(poly):
    ...
    # Flat [x1,y1,x2,y2,...]
    if isinstance(poly, list) and len(poly) >= 6 and isinstance(poly[0], (int, float)):
        try:
...
class ToothV3Dataset(Dataset):
    def __init__(self, split):
        ...
        for r in d["records"]:
            ...
            for x in r.get("instances", []):
                ...
                poly = normalize_polygon(x.get("polygon"))
    ...
    def __getitem__(self, idx):
        ...
        for x in r["instances"]:
            ...
            mask = torch.as_tensor(__import__("numpy").array(mask_img), dtype=torch.uint8)
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            for out, target in zip(outputs, targets):
                ...
                preds = [b for b, s in zip(pred_boxes, pred_scores) if s >= score_threshold]
                ...
                for gb in gt_boxes:
                    ...
                    if best_j is not None and best >= iou_threshold:
                        matched_total += 1
                        precision_tp += 1
                        used.add(best_j)

    recall = matched_total / max(gt_total, 1)

    precision = precision_tp / max(prediction_total, 1)
...
def extract_state_dict(ckpt):
    ...
    raise RuntimeError("Could not locate model state_dict")
...
def main():
    ...
    train_ds = ToothV3Dataset("train")

    val_ds = ToothV3Dataset("validation")
    ...
    print("Train tooth instances:", sum(train_ds.instance_counts.values()))

    print("Validation tooth instances:", sum(val_ds.instance_counts.values()))

    print("Skipped train instances without polygon:", train_ds.skipped_no_polygon)

    print("\nTRAIN SOURCES")

    for k, v in train_ds.source_counts.items():
        print(f"{k:40}", v, "images |", train_ds.instance_counts[k], "teeth")
    ...
    if len(train_ds) == 0:
        raise RuntimeError("No usable training records.")

    model = make_model()

    if not OLD_CKPT.exists():
        raise FileNotFoundError(f"Missing Tooth V2 checkpoint: {OLD_CKPT}")
    ...
    for epoch in range(4):
        ...
        metrics = evaluate_recall(
            model, val_loader, device, score_threshold=0.50, iou_threshold=0.50
        )

        # Detection recall remains primary,
        # but precision prevents prediction explosion.
        score = 0.70 * metrics["recall"] + 0.30 * metrics["f1"]
        ...
        print(
            f"  matched={metrics['matched']}/{metrics['gt']} predictions={metrics['predictions']}"
        )
        ...
        state = {
            ...
            "metrics": metrics,
            "score": score,
            "source_checkpoint": str(OLD_CKPT),
            "dataset": "dentai_v3_super",
        }
