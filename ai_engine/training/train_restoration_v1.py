...
def evaluate(model, loader, device):
    ...
    per_class = {CLASSES[i]: cc[i] / ct[i] if ct[i] else 0 for i in range(len(CLASSES))}
...
def main():
    ...
    train_ds = RestorationDataset("train", train=True)

    val_ds = RestorationDataset("validation", train=False)
    ...
    class_weights = torch.tensor([1.0, 2.5], dtype=torch.float32, device=device)
    ...
    out = Path("checkpoints/restoration_v1")
    ...
    print("MODEL:", "checkpoints/restoration_v1/best.pt")
    print("====================================")
