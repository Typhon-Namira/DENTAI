...
OLD_CKPT = Path("checkpoints/fdi_v2_final/fdi_v2_best_90_38.pt")

OUT = Path("checkpoints/fdi_v3_fixed")
...
class FDIV3Dataset(Dataset):
    def __init__(self, split, train=False):
        d = json.loads((ROOT / f"{split}.json").read_text(encoding="utf-8"))
        ...
        for r in d["records"]:
            ...
            for x in r.get("instances", []):
                ...
                fdi = str(x.get("fdi_number", "")).strip()
    ...
    def __getitem__(self, idx):
        ...
        return (self.tf(crop), spatial, s["label"])
...
def main():
    ...
    train_ds = FDIV3Dataset("train", train=True)

    val_ds = FDIV3Dataset("validation", train=False)

    counts = Counter(s["fdi"] for s in train_ds.samples)
    ...
    print()
    print("✓ FDI V2 checkpoint loaded STRICTLY")
    print("V2 epoch:", ckpt.get("epoch"))
    print("V2 val_acc:", ckpt.get("val_acc"))
    ...
    baseline_acc, baseline_macro, _ = evaluate(model, val_loader, device)
    ...
    print("Baseline val_acc:", round(baseline_acc, 4))

    print("Baseline macro_recall:", round(baseline_macro, 4))
    ...
    if baseline_acc < 0.65:
        raise RuntimeError(
            "Baseline too low. Stop: dataset/preprocessing is inconsistent with FDI V2."
        )
    ...
    for s in train_ds.samples:
        ...
        sample_weights.append((max_count / max(n, 1)) ** 0.25)
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": model.spatial_net.parameters(), "lr": 3e-5},
    ...
    best_score = 0.70 * baseline_acc + 0.30 * baseline_macro
    ...
    torch.save(baseline_state, OUT / "best.pt")
    ...
    for epoch in range(5):
        ...
        val_acc, macro, _ = evaluate(model, val_loader, device)

        score = 0.70 * val_acc + 0.30 * macro
        ...
        if score > best_score:
            ...
            print("*** NEW BEST:", round(best_score, 4), "***")
    ...
    print("BEST VAL ACC:", best["val_acc"])

    print("BEST MACRO:", best["macro_recall"])
