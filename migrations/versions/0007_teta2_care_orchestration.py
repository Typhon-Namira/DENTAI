...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("branch_id", sa.Uuid(), sa.ForeignKey("branches.id", ondelete="CASCADE"), nullable=False, unique=True),
        ...
        sa.Column("auto_outreach_after_review", sa.Boolean(), nullable=False, server_default=sa.true()),
    ...
    op.create_table(
        ...
        sa.Column("analysis_id", sa.Uuid(), sa.ForeignKey("ai_analyses.id", ondelete="CASCADE"), nullable=False, unique=True),
    ...
    op.create_table(
        "care_plan_items",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column("care_plan_id", sa.Uuid(), sa.ForeignKey("care_plans.id", ondelete="CASCADE"), nullable=False),
        sa.Column("finding_id", sa.Uuid(), sa.ForeignKey("dental_findings.id", ondelete="CASCADE"), nullable=False, unique=True),
    ...
    for name in ("patient_id", "care_plan_id", "branch_id", "whatsapp_phone", "status", "last_message_at"):
        op.create_index(f"ix_care_conversations_{name}", "care_conversations", [name])

    op.create_table(
        "care_conversation_messages",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column("conversation_id", sa.Uuid(), sa.ForeignKey("care_conversations.id", ondelete="CASCADE"), nullable=False),
        sa.Column("care_plan_item_id", sa.Uuid(), sa.ForeignKey("care_plan_items.id", ondelete="SET NULL")),
    ...
    for name in ("conversation_id", "care_plan_item_id", "direction", "status", "provider_message_id", "created_at"):
        op.create_index(f"ix_care_conversation_messages_{name}", "care_conversation_messages", [name])
    ...
    op.create_table(
        ...
        sa.Column("branch_id", sa.Uuid(), sa.ForeignKey("branches.id"), nullable=False),
        sa.Column("doctor_id", sa.Uuid(), sa.ForeignKey("users.id")),
        sa.Column("conversation_id", sa.Uuid(), sa.ForeignKey("care_conversations.id", ondelete="SET NULL")),
        sa.Column("care_plan_item_id", sa.Uuid(), sa.ForeignKey("care_plan_items.id", ondelete="SET NULL")),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint("branch_id", "starts_at", "patient_id", name="uq_care_appointment_patient_slot"),
    )
    for name in ("patient_id", "branch_id", "doctor_id", "conversation_id", "care_plan_item_id", "starts_at", "ends_at", "status"):
