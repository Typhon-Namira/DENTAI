...
export function CareCommandCenter() {
  ...
  const nav = createPortal(
    <div className="care-nav-cluster" aria-label="Teta2 AI">
      <div className="care-nav-label"><span className="care-nav-orb"><Sparkles /></span><b>Teta2 AI</b><i>{hy ? "ԻՆՔՆԱՎԱՐ" : "AUTONOMOUS"}</i></div>
  ...
  const workspace = active ? createPortal(
    <section className="care-integrated-workspace">
      <header className="care-integrated-head">
        <div className="care-ai-identity"><AIOrb /><div><small>TETA2 · CLINICAL ORCHESTRATION</small><h1>{tab === "overview" ? (hy ? "AI խնամքի հոսք" : "AI care workflow") : tab === "appointments" ? (hy ? "Չեքափի ամրագրումներ" : "Check-up appointments") : tab === "messages" ? (hy ? "Պացիենտների AI զրույցներ" : "Patient AI conversations") : (hy ? "AI կանոններ և հիշողություն" : "AI schedule & memory")}</h1><p>{hy ? "Բժիշկը վերահսկում է կլինիկական որոշումները։ Teta2-ը կազմակերպում է մնացած հոսքը։" : "You control clinical decisions. Teta2 coordinates the workflow, outreach, conversation and booking."}</p></div></div>
...
function Automation({ lang, branches, branchId, setBranchId, settings, setSettings, onSaved }: { lang:Lang; branches:BranchSummary[]; branchId:string; setBranchId:(id:string)=>void; settings:CareSettings|null; setSettings:(s:CareSettings)=>void; onSaved:()=>Promise<void> }) {
  ...
  async function save(){if(!settings)return;setBusy(true);setError("");try{const {id:_id,branch_id:_branch,...body}=settings;setSettings(await productApi.updateCareSettings(branchId,body));await onSaved()}catch(r){setError(errorMessage(r))}finally{setBusy(false)}}
