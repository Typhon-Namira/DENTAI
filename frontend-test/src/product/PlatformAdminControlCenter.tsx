...
function EditClinicModal({ clinic, busy, close, save }: { clinic: Clinic; busy: string; close: () => void; save: (body: Record<string, unknown>) => Promise<void> }) {
  ...
  const [state, setState] = useState(clinic.subscription_state === "ARCHIVED" ? "SUSPENDED" : clinic.subscription_state);
  const [expiry, setExpiry] = useState(clinic.subscription_expires_at ? clinic.subscription_expires_at.slice(0, 10) : "");
  return <Modal title="Edit clinic account" close={close}><div className="pa3-modal-form"><div className="pa3-note">Archive is intentionally separated from account editing. Use the dedicated Archive action when access must be disabled while preserving clinical data.</div><label>Clinic name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Subscription state<select value={state} onChange={(event) => setState(event.target.value)}><option>ACTIVE</option><option>PAYMENT_REVIEW</option><option>EXPIRED</option><option>SUSPENDED</option></select></label><label>Expiry date<input type="date" value={expiry} onChange={(event) => setExpiry(event.target.value)} /></label><button className="pa3-primary" disabled={!!busy} onClick={() => void save({ name, subscription_state: state, expires_at: expiry ? new Date(`${expiry}T23:59:59Z`).toISOString() : null })}><Save /> Save account</button></div></Modal>;
}
